...
export default async function ResourcesPage() {
  ...
  const rows = await db
    ...
    .where(eq(modules.courseId, access.courseId))
    .orderBy(asc(modules.position), asc(lessons.position), asc(resources.position));
      ...
      <main>
        <div className="app-page">
          ...
          <p className="text-[var(--muted)] mb-6">
            All your guides, prompts, tools, and supporting materials in one place.
          </p>
          <ResourceBrowser items={rows} />
        </div>
      </main>
    </>
  );
