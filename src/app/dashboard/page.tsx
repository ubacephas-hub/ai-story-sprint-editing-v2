  ...
  return <><Navbar user={user}/><main><div className="app-page">
    ...
    {active&&<>
      ...
      <div className="grid md:grid-cols-2 gap-3">{courseModules.map((m,i)=>{const done=m.lessons.filter(x=>x.progress?.status==="completed").length;const state=done===m.lessons.length?"Completed":done?"In progress":"Not started";return <details className="module-outline" key={m.module.id} open={i===courseModules.findIndex(x=>x.lessons.some(l=>l.progress?.status==="in_progress"))}>
        <summary><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><div className="module-icon mb-0">{icons[i % icons.length]}</div><div><strong className="text-sm text-[var(--muted)]">Module {i+1}</strong><h3 className="font-bold text-lg">{m.module.title}</h3><p className="text-xs text-[var(--muted)]">{done} of {m.lessons.length} lessons</p></div></div><div className="flex items-center gap-3"><span className={`badge ${state.toLowerCase().replace(" ","-")}`}>{state}</span><span className="module-chevron">›</span></div></div></summary>
