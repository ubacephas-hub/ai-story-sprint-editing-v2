...
export default async function DashboardPage() {
  ...
  return <><Navbar user={user}/><main><div className="app-page">
    ...
    {active&&<>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3">{courseModules.map((m,i)=>{const done=m.lessons.filter(x=>x.progress?.status==="completed").length;const state=done===m.lessons.length?"Completed":done?"In progress":"Not started";return <Link href={m.lessons[0]?`/lesson/${m.lessons[0].lesson.id}`:"#"} className="soft-card module-card text-[var(--ink)]" key={m.module.id}><div className="module-icon">{icons[i]}</div><strong>Module {i+1}</strong><h3 className="font-bold">{m.module.title}</h3><div className="module-meta"><span className={`badge ${state.toLowerCase().replace(" ","-")}`}>{state}</span><span>→</span></div><p className="text-xs text-[var(--muted)] mt-2">{done} of {m.lessons.length} lessons</p></Link>})}</div>
