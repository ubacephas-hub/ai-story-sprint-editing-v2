...
export default async function AccountPage() {
  ...
  const [access] = await db
    ...
    .where(eq(courseAccess.userId, session.user.id))
    .limit(1);
  ...
  const completed = completedProgressCount(progressRows);
  const percentage = overallProgressPercentage(progressRows);
    ...
    <>
      ...
      <main>
        <div className="app-page">
          <p className="page-eyebrow">Profile and access</p>
          <h1 className="text-3xl font-bold mt-1 mb-6">My Account</h1>
          <div className="grid lg:grid-cols-[260px_1fr] gap-4">
            <aside className="soft-card h-fit">
              <div className="w-16 h-16 rounded-full bg-[#ede8ff] text-[var(--brand)] grid place-items-center text-2xl font-bold mb-3">
                {session.user.name.slice(0, 1).toUpperCase()}
              </div>
              <strong>{session.user.name}</strong>
              <p className="text-sm text-[var(--muted)]">{session.user.email}</p>
              <hr className="my-4 border-[var(--line)]" />
              <span className={`badge ${access?.status || "pending"}`}>
                {access?.status || "No enrollment"}
              </span>
            </aside>
            <section className="soft-card">
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                ...
                <div>
                  <span className="text-sm text-[var(--muted)]">Account status</span>
                  <p><span className="badge active">{session.user.accountStatus}</span></p>
                </div>
                <div>
                  <span className="text-sm text-[var(--muted)]">Course</span>
                  <p className="font-semibold">{access?.courseTitle || "Not enrolled"}</p>
                </div>
                <div>
                  <span className="text-sm text-[var(--muted)]">Course progress</span>
                  <p className="font-semibold">
                    {Math.round(percentage)}% · {completed} of {progressRows.length} lessons completed
                  </p>
                </div>
                <div>
                  <span className="text-sm text-[var(--muted)]">Member since</span>
                  <p className="font-semibold">
                    {access?.createdAt ? new Date(access.createdAt).toLocaleDateString() : "—"}
                  </p>
                </div>
              </div>
