...
export default async function LessonPage({ params }: LessonPageProps) {
  ...
  return (
    <><Navbar user={user}/><main><div className="app-page">
      ...
      {lessonResources.length?<div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">{lessonResources.map(res=><article key={res.id} className="soft-card resource-tile"><span className="resource-icon">{res.type==="link"?"↗":res.type==="text"?"☷":"□"}</span><span className="page-eyebrow">{res.type}</span><strong>{res.title}</strong>{res.description&&<p className="text-sm text-[var(--muted)]">{res.description}</p>}{res.type==="text"&&res.content&&<p className="text-sm mt-2">{res.content}</p>}<div className="resource-action">{res.type==="link"&&res.url&&<a className="btn secondary small" target="_blank" rel="noopener noreferrer" href={res.url}>Open resource ↗</a>}{res.type==="document"&&res.filePath&&<a className="btn secondary small" href={`/api/resources/${res.id}/download`}>Open document</a>}</div></article>)}</div>:<div className="soft-card text-center text-[var(--muted)]">No resources have been added for this lesson yet.</div>}
